...
.edit-files-link svg { width: 16px; height: 16px; }${mediaButtonCss}`;
  ...
  getChangedConfig(config, serverOnly = false) {
    ...
    // Display keys that are auto-generated from byte values (exclude from JS config)
    const autoGeneratedDisplayKeys = [
      "perFileMaxSizeDisplay",
      "perFileMaxSizePerTypeDisplay",
      "perTypeMaxTotalSizeDisplay",
      "totalMaxSizeDisplay",
    ...
    for (const [groupKey, groupValue] of Object.entries(config)) {
      ...
      if (groupValue && typeof groupValue === "object" && !Array.isArray(groupValue)) {
        ...
        for (const [optionKey, optionValue] of Object.entries(groupValue)) {
          ...
          // For JS config (not serverOnly), skip auto-generated display keys
          if (!serverOnly && autoGeneratedDisplayKeys.includes(optionKey)) continue;
